...
print(
    f"Date range of gold prices avalable from -{df.loc[:, 'Date'][0]} to {df.loc[:, 'Date'][len(df)-1]}")
...
df_1 = df.groupby(df.index.year).mean().rename(
    columns={'Price': 'yearly_mean'})
df_1 = df_1.merge(df.groupby(df.index.year).std().rename(
    columns={'Price': 'yearly_std'}), left_index=True, right_index=True)
df_1['cov_pct'] = (
    (df_1['yearly_std'] / df_1['yearly_mean']) * 100
).round(2)
df_1.head()
...
def mape(actual, pred): return round(
    np.mean(np.abs((actual - pred) / actual)) * 100, 2)


mape_model1_test = mape(test['price'].values, test_predictions_model1)
print(f"mape is {mape_model1_test:.3f}%", "%")
result = pd.DataFrame(
    {'test mape(%)': [mape_model1_test]}, index=["RegressionTime"])
...
std_error = np.std(final_model.resid, ddof=1)

pred_df = pd.DataFrame({
    'lower_CI': prediction - 1.96 * std_error,
    'prediction': prediction,
    'upper_CI': prediction + 1.96 * std_error
})
